...
public class ProjectDAO implements IProjectDAO {
        ...
        try {
            ...
            while(resultSet.next()) {
                Project project = new Project(resultSet.getString("NombreProyecto"),
                        ...
                        resultSet.getString("Matricula"), resultSet.getString("idResponsableProyecto"));
    ...
    public Project getProject(int idProject) {
        ...
        try {
            ...
            resultSet = preparedStatement.executeQuery();
            while(resultSet.next()) {
                project = new Project(resultSet.getString("NombreProyecto"),
                        resultSet.getString("Descripcion"), resultSet.getString("Recursos"),
                        resultSet.getString("Matricula"), resultSet.getString("idResponsableProyecto"));
            }
        } catch (SQLException sqlException) {
    ...
    public boolean addProject(Project project) {
        ...
        try {
            connection = mySQLConnection.getConnection();
            String sql = "INSERT INTO proyecto(NombreProyecto,Descripcion,Recursos,Matricula,NumeroPersonalCoordinador) VALUES(?,?,?,?,?)";
            ...
            preparedStatement.setString(4,project.getStudentEnrollment());
            preparedStatement.setString(5,project.getProfessorNumberPersonal());
            int numberRowsAffected = preparedStatement.executeUpdate();
    ...
    public boolean updateProject(Project project) {
        ...
        try {
            connection = mySQLConnection.getConnection();
            String sql = "UPDATE proyecto SET Descripcion=?, Recursos=?, Matricula=?, NumeroPersonalCoordinador=? WHERE NombreProyecto=?";
            preparedStatement = connection.prepareStatement(sql);
            ...
            preparedStatement.setString(4,project.getProfessorNumberPersonal());
            int numberRowsAffected = preparedStatement.executeUpdate();
            result = (numberRowsAffected > 0);
        } catch (SQLException sqlException) {
            Logger.getLogger(ProjectDAO.class.getName()).log(Level.SEVERE, sqlException.getMessage(), sqlException);
    ...
    public boolean deleteProject(Project project) {
        ...
        try {
            connection = mySQLConnection.getConnection();
            String sql = "DELETE FROM proyecto WHERE NombreProyecto = ?";
            preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1,project.getProjectName());
            int numberRowsAffected = preparedStatement.executeUpdate();
            result = (numberRowsAffected > 0);
